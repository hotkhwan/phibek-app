...
<DomainStarter title="AI Investigation Center" subtitle="ค้นหาบุคคล รถ เหตุการณ์ และพฤติกรรมจากกล้องวงจรปิดด้วย AI" icon="bi-search-heart" legacyName="ksearch">
  ...
  {#if zoomCard}<div class="zoom-modal" role="button" tabindex="0" aria-label="Close zoom image" onclick={(e) => { if (e.target === e.currentTarget) closeZoom() }} onkeydown={(e) => { if (e.key === 'Escape') closeZoom(); if (e.key === 'ArrowLeft') moveZoom(-1); if (e.key === 'ArrowRight') moveZoom(1) }}><div><header><span>{zoomCard.title}</span><button type="button" aria-label="Close zoom image" onclick={closeZoom}><i class="bi bi-x-lg"></i></button></header><div class="zoom-stage"><ProtectedImage src={previewUrl(zoomCard)} alt={zoomCard.caption} class="zoom-image" bbox={zoomCard.media?.bbox} /><div class="zoom-nav"><button type="button" aria-label="Previous image" disabled={zoomIndex <= 0} onclick={() => moveZoom(-1)}><i class="bi bi-chevron-left"></i></button><button type="button" aria-label="Next image" disabled={zoomIndex >= cards.length - 1 && !hasNextPage} onclick={() => moveZoom(1)}><i class="bi bi-chevron-right"></i></button></div></div></div></div>{/if}
</DomainStarter>
...
<style lang="scss">
  ...
  .event-card { overflow: hidden; .media-box { position: relative; display: block; width: 100%; aspect-ratio: 16 / 9; overflow: hidden; background: #090a0d; border: 0; padding: 0; color: inherit; cursor: default; } .media-button { cursor: zoom-in; } .media-button:hover .zoom-btn, .media-button:focus-visible .zoom-btn { border-color: rgba(0,208,132,.72); color: #00d084; background: rgba(0,0,0,.48); } :global(.event-image) { width: 100%; height: 100%; object-fit: cover; background: #090a0d; } .confidence { position: absolute; z-index: 2; top: .75rem; left: .75rem; background: rgba(0,0,0,.72); color: #fff; border-radius: 5px; padding: .25rem .5rem; font-size: .75rem; pointer-events: none; } .zoom-btn { position: absolute; z-index: 2; top: .75rem; right: .75rem; width: 36px; height: 36px; border-radius: 50%; border: 1px solid rgba(255,255,255,.25); color: rgba(255,255,255,.85); background: rgba(0,0,0,.28); backdrop-filter: blur(8px); display: grid; place-items: center; pointer-events: none; } }
  ...
  .zoom-modal { position: fixed; inset: 0; z-index: 80; background: rgba(0,0,0,.82); display: grid; place-items: center; padding: 1rem; > div { width: min(1100px, 96vw); border: 1px solid rgba(255,255,255,.14); border-radius: 8px; background: #0f1014; overflow: hidden; } header { display: flex; align-items: center; justify-content: space-between; gap: 1rem; padding: .75rem 1rem; border-bottom: 1px solid rgba(255,255,255,.12); span { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } } button { border: 0; background: transparent; color: #fff; } .zoom-stage { position: relative; display: grid; place-items: center; min-height: 320px; background: #000; } .zoom-nav { position: absolute; z-index: 3; top: .85rem; right: .85rem; display: inline-flex; overflow: hidden; border: 1px solid rgba(255,255,255,.22); border-radius: 999px; background: rgba(0,0,0,.38); backdrop-filter: blur(8px); button { width: 42px; height: 38px; display: grid; place-items: center; border-left: 1px solid rgba(255,255,255,.14); &:first-child { border-left: 0; } &:disabled { opacity: .35; cursor: not-allowed; } } } :global(.zoom-image) { width: 100%; height: min(78vh, 760px); object-fit: contain; background: #000; } }
