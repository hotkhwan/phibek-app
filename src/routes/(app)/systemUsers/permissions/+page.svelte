...
<div class="permission-workspace">
  <aside class="policy-list">
    <div class="btn-group btn-group-sm mb-3">
      <button class="btn" class:btn-theme={activeTab === 'resource'} class:btn-outline-theme={activeTab !== 'resource'} onclick={() => switchTab('resource')}>Resource</button>
      <button class="btn" class:btn-theme={activeTab === 'menu'} class:btn-outline-theme={activeTab !== 'menu'} onclick={() => switchTab('menu')}>Menu</button>
      <button class="btn" class:btn-theme={activeTab === 'api'} class:btn-outline-theme={activeTab !== 'api'} onclick={() => switchTab('api')}>API</button>
    </div>
    <input class="form-control form-control-sm mb-3" bind:value={search} placeholder="Search profiles..." />
    {#if activeTab === 'api'}
      <div class="empty-panel">API permission management is read-only here.</div>
    {:else}
      <div class="policy-stack">
        {#each filteredRows as row (row.id)}
          <button type="button" class="policy-row" class:active={selectedId === row.id} onclick={() => { selectedId = row.id; hydrate(row.id) }}>
            <span class="policy-status" class:on={(row as ResourcePermission).status !== false}></span>
  ...
  <main class="policy-editor">
    {#if activeTab === 'api'}
      <div class="empty-panel h-100 d-flex align-items-center justify-content-center">API permissions are separated from menu/resource profiles to avoid schema mixing.</div>
    {:else}
      <div class="policy-toolbar">
        <div><div class="text-muted small text-uppercase">{createMode ? 'New policy profile' : selected ? 'Edit policy profile' : 'No profile selected'}</div><h2>{form.name || selected?.name || 'Untitled profile'}</h2></div>
        <div class="d-flex gap-2">
...
<style lang="scss">
  ...
  .policy-list, .policy-editor, .policy-card { border: 1px solid rgba(var(--bs-theme-rgb), .24); background: rgba(var(--bs-body-bg-rgb), .28); border-radius: .35rem; }
  .policy-list, .policy-editor { padding: 1rem; min-height: 0; }
  .policy-stack, .choice-list { display: grid; gap: .5rem; max-height: calc(100vh - 19rem); overflow: auto; padding-right: .25rem; }
  .policy-row, .choice-row { width: 100%; border: 1px solid rgba(var(--bs-border-color-rgb), .7); background: rgba(var(--bs-body-bg-rgb), .2); color: var(--bs-body-color); border-radius: .35rem; padding: .65rem .75rem; display: flex; align-items: center; gap: .6rem; text-align: left; }
  .policy-row.active, .choice-row.selected { border-color: rgba(var(--bs-theme-rgb), .7); background: rgba(var(--bs-theme-rgb), .14); }
  .policy-status { width: .55rem; height: .55rem; border-radius: 50%; background: var(--bs-secondary); box-shadow: 0 0 0 .2rem rgba(var(--bs-secondary-rgb), .12); }
  .policy-status.on { background: var(--bs-theme); box-shadow: 0 0 0 .2rem rgba(var(--bs-theme-rgb), .15); }
  .policy-toolbar, .policy-summary { display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap; }
  .policy-toolbar h2 { font-size: 1.25rem; margin: 0; }
  .policy-form { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: .75rem; margin-block: 1rem; }
  ...
  .policy-card { padding: .85rem; min-height: 21rem; }
  .policy-card header { display: flex; align-items: center; justify-content: space-between; gap: .75rem; font-weight: 700; margin-bottom: .75rem; }
  ...
  .policy-summary { border-top: 1px solid rgba(var(--bs-theme-rgb), .24); margin-top: 1rem; padding-top: .85rem; color: rgba(var(--bs-body-color-rgb), .7); }
  @media (max-width: 1199.98px) { .permission-workspace, .policy-grid, .policy-form { grid-template-columns: 1fr; } }
</style>
